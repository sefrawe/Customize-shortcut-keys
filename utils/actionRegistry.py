    ...
    ActionDef(
        ...
        params=[
            ParamSpec(
                ...
                default="",
                required=True,
                placeholder="如: ctrl+shift+s ，单独的加号写成 plus"
            )
        ],
        show_in_shortcut=False,  # 【关键】主快捷键编辑窗不显示
        show_in_action_group=True,  # 【关键】仅在动作组步骤下拉框中可选
        hint=(
            "模拟按键是动作组这个动作的补充,意思是向当前活动窗口发送指定的单个按键组合。\n\n"
            "• 格式：按键用 + 连接，如 ctrl+s 、shift+f5 、numpad_1 ，单独的加号写成 plus\n"
            "• ⚠ 此处没有格式验证：写错键名不会在保存时报错。\n"
            "  当前版本正式执行时这个功能还没做报错弹窗,只做了控制台print,因此也不会有提示,"
            "所以慎重选择单步失败时的选项\n"
            "  建议添加后先点「▶ 试运行」确认无误，再正式启用本步骤！\n"
            "• 连续多组按键：复制本步骤后修改按键名，配合每步的延迟控制节奏\n"
            "• 中文输入法下字母键可能被输入法接管变候选词；指令型按键(ctrl+s、F5、alt+f4 等)\n"
            "  一般不受影响。输入大段文字请使用\"粘贴文本\"（走剪贴板，不经输入法）。\n"
            "  遇到需要切换输入法的场景：切输入法按键 → 延迟300ms → 本动作 → 切回来\n"
            "• 向管理员权限的程序发送可能无效\n"
            "• 每次发送前会先释放你物理正在按住的修饰键，防止模拟的按键组合被污染"
        )
    ),
    # =============================================================================
