...
组件依次是名字输入框、快捷键输入框、备注输入框、动作类型下拉框、动作参数区域、
文本框（不可输入）、
搜索按钮、取消按钮

结果应该是一个列表，显示所有匹配的快捷键信息
根据实现方式来看应该叫条件筛选窗口：默认显示所有快捷键，根据用户输入的条件进行筛选，显示匹配的快捷键信息
'''
...
class ShortcutSearchWindow(ctk.CTkToplevel):
    def __init__(self, parent,config):  # 参数分别为父窗口和要编辑的快捷键方案的配置
        ...
        self.geometry("600x600")  # 设置窗口初始尺寸为600x400
        self.title("搜索快捷键（此窗口无法实时获取最新配置，仅点击搜索时获取）:根据快捷键的名字、快捷键组合、备注、动作类型、动作参数进行搜索")  # 设置窗口标题



        self.grid_columnconfigure(0, weight=1)
        self.grid_rowconfigure(0, weight=1)  # 让输入框列可伸缩
        ...
        #名字输入框
        self.nameLabel = ctk.CTkLabel(self.scrollFrame, text="名字:",font=("微软雅黑", 16))
        ...
        self.scrollFrame.grid_columnconfigure(1, weight=1)  # 让输入框列可伸缩


        #快捷键输入框
        self.keyLabel = ctk.CTkLabel(self.scrollFrame, text="快捷键:",font=("微软雅黑", 16))
        self.keyLabel.grid(row=1, column=0, sticky="w", padx=5, pady=5)
        ...
        #备注输入框（多行文本框）
        self.descriptionLabel = ctk.CTkLabel(self.scrollFrame, text="备注:",font=("微软雅黑", 16))
        self.descriptionLabel.grid(row=2, column=0, sticky="w", padx=5, pady=5)
        self.descriptionEntry = ctk.CTkTextbox(self.scrollFrame, font=("微软雅黑", 14), wrap="word", height=100)
        ...
        # 结果显示（多行文本框, 只读,自动占据可滚动框架剩余区域）
        self.descriptionLabel = ctk.CTkLabel(self.scrollFrame, text="搜索结果:", font=("微软雅黑", 16))
        self.descriptionLabel.grid(row=5, column=0, sticky="w", padx=5, pady=5)
        self.descriptionEntry = ctk.CTkTextbox(self.scrollFrame, font=("微软雅黑", 14), wrap="word")
        self.descriptionEntry.grid(row=5, column=1, sticky="nsew", padx=5, pady=5)

        self.scrollFrame.grid_rowconfigure(5, weight=1)  # 让结果框列可伸缩
        #让文本框无法输入
    ...
    def onSearch(self):
        """点击搜索按钮时触发：收集数据、搜索、显示结果"""
        pass
        # # 1. 收集基本数据
        # name = self.nameEntry.get().strip()
        # key = self.keyEntry.get().strip()
        # desc = self.descriptionEntry.get("1.0", "end-1c").strip()  # 获取多行文本框的内容，去掉末尾换行符
        #
        # # 2. 搜索匹配的快捷键
        # matchedShortcuts = []
        # for shortcut in shortcuts:
        #     if name and name not in shortcut.get("name", ""):
        #         continue
        #     if key and key not in shortcut.get("keyCombination", ""):
        #         continue
        #     if desc and desc not in shortcut.get("description", ""):
        #         continue
        #     matchedShortcuts.append(shortcut)
        #
        # # 3. 显示结果
        # self.descriptionEntry.configure(state="normal")  # 允许编辑以清空内容
        # self.descriptionEntry.delete("1.0", "end")  # 清空结果框
        # if matchedShortcuts:
        #     for sc in matchedShortcuts:
        #         self.descriptionEntry.insert("end",
        #                                      f"ID: {sc.get('id', '')}, 名字: {sc.get('name', '')}, 快捷键: {sc.get('keyCombination', '')}, 备注: {sc.get('description', '')}, 动作类型: {sc.get('action', '')}, 动作参数: {sc.get('actionParams', '')}\n")
        # else:
        #     self.descriptionEntry.insert("end", "未找到匹配的快捷键。")
        # self.descriptionEntry.configure(state="disabled")  # 禁止编辑
